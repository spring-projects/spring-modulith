...
* `spring-modulith-example-outbox` -- shows the outbox pattern for event externalization, where events are persisted to an outbox table within the same transaction and later processed asynchronously.
