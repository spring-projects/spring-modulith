...
class CouchbaseEventPublicationRepositoryTest {
	...
	static abstract class TestBase {
		...
		CouchbaseEventPublicationRepository repository;
		CompletionMode completionMode;
		String archiveCollection = CouchbaseEventPublicationRepository.ARCHIVE_COLLECTION;

		@BeforeEach
		void setUp() {
			this.completionMode = CompletionMode.from(environment);
			this.repository = new CouchbaseEventPublicationRepository(couchbaseTemplate, completionMode);
		}

		@AfterEach
		void tearDown() {
			couchbaseTemplate.removeByQuery(CouchbaseEventPublication.class).all();
			couchbaseTemplate.removeByQuery(CouchbaseEventPublication.class).inCollection(archiveCollection).all();
		}
		...
		void marksPublicationAsCompletedById() {
			...
			if (completionMode == CompletionMode.ARCHIVE) {
				assertThat(couchbaseTemplate.findByQuery(CouchbaseEventPublication.class).inCollection(archiveCollection).all()).isNotEmpty();
			}
		}
		...
		void shouldDeleteCompletedEvents() {
			...
			assertThat(couchbaseTemplate.findByQuery(CouchbaseEventPublication.class).all()) //
					.hasSize(1) //
					.element(0) //
		...
		private void savePublicationAt(LocalDateTime date) {
			...
			var publication = new CouchbaseEventPublication(UUID.randomUUID(), now, "", "", null, Status.PUBLISHED, now, 1);

			couchbaseTemplate.save(publication);
		}
