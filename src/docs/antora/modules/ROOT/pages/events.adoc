...
This especially makes the component harder to test as we need to have instances available of those dependent beans just to create an instance of `OrderManagement` (see xref:testing.adoc#efferent-dependencies[Dealing with Efferent Dependencies]).
...
If you want to customize this, check out the xref:appendix.adoc#configuration-properties[ `spring.modulith.events.registry-trigger-annotation` property].
...
[[publication-registry.lifecycle]]
...
[[publication-registry.lifecycle.states]]
==== Publication states
...
* `COMPLETED` – The listener finished successfully. A completion date is set (unless the completion xref:events.adoc#publication-registry.completion[mode is `DELETE`]).
* `FAILED` – The listener threw an exception, or the publication was marked failed by the staleness mechanism (see <<publication-registry.lifecycle.staleness>>).
* `RESUBMITTED` – A previously failed publication was resubmitted and is again pending processing.
* `ABANDONED` (since 2.2) – The listener invocation failed and an `AbandonPolicy` decided the publication should permanently give up on further resubmission (see <<publication-registry.lifecycle.abandoning>>). Distinct from `COMPLETED`, which indicates a successful listener invocation.

[plantuml, event-publication-lifecycle, format='svg']
....
@startuml
state PUBLISHED
state PROCESSING
...
[*] -right-> PUBLISHED : event stored
PUBLISHED -down-> PROCESSING : listener picks up
PROCESSING -right-> COMPLETED : success
PROCESSING -down-> FAILED : exception
PROCESSING -right-> ABANDONED : exception, abandon policy applies
FAILED -left-> RESUBMITTED : resubmit
RESUBMITTED -right-> PROCESSING : resubmission triggered
@enduml
....

[[publication-registry.lifecycle.details]]
==== Publication details
...
[[publication-registry.lifecycle.staleness]]
...
So that they can be treated as failed and resubmitted (or ignored), you can configure after which duration each of these states is considered _stale_. Stale publications are periodically marked as `FAILED` by a background task.

Spring Modulith provides a _Staleness Monitor_ (since 2.0) that runs as a scheduled task at a configurable interval.
...
You customize it via the xref:appendix.adoc#configuration-properties[`spring.modulith.events.staleness` configuration properties].
...
[[publication-registry.lifecycle.failed-and-resubmission]]
...
[[publication-registry.lifecycle.abandoning]]
==== Abandoning failed publications (since 2.2)

Some failures are not worth resubmitting at all -- for example, a payload that can never be deserialized, or a permanent error signaled by a downstream system. For those cases, a failing publication can be marked `ABANDONED` right away instead of `FAILED`, so it is no longer picked up by the resubmission mechanisms described in <<publication-registry.lifecycle.failed-and-resubmission>> or considered by the Staleness Monitor.

The decision is made by an `AbandonPolicy`:
...
`Decision` is one of `ABANDON`, `RETAIN`, or `DEFAULT`. Register as many `AbandonPolicy` beans as you need; they are consulted in the order defined by `@Order` / `Ordered`, and the first one to return `ABANDON` or `RETAIN` wins, short-circuiting the remaining policies. If every policy returns `DEFAULT` (or none is configured at all), the framework-provided default applies: a configurable number of completion attempts, controlled via the xref:appendix.adoc#configuration-properties[`spring.modulith.events.resubmission.abandon-after-attempts`] property. That property defaults to `-1`, which disables the framework-provided default so that, without any `AbandonPolicy` bean registered, publications are never abandoned and behave exactly as before 2.2.

The decision is made only at the moment a publication actually fails, i.e. when a listener invocation throws. It is not applied retroactively to publications that are already `FAILED` in the store. As a result, introducing or tightening an `AbandonPolicy` never abandons publications that failed before that change was deployed -- it only affects failures happening from that point on.

`AbandonedEventPublications` -- available as a Spring bean analogous to `CompletedEventPublications` -- lets you inspect and purge abandoned publications, as described in <<publication-registry.managing-publications>>.
...
|Couchbase
|`spring-modulith-starter-couchbase`
|Using Couchbase as persistence technology. Also enables Couchbase transactions. The transaction auto-configuration can be disabled by setting the `spring.modulith.events.couchbase.transaction-management.enabled` property to `false`.
...
* `CompletedEventPublications` -- This interface allows accessing all completed event publications, and provides an API to immediately purge all of them from the database or the ones older than a given duration (for example, 1 minute).
...
* `FailedEventPublications` (since 2.0) -- This interface allows resubmitting only failed event publications via `resubmit(ResubmissionOptions)`, as described in <<publication-registry.lifecycle.failed-and-resubmission>>.
* `AbandonedEventPublications` (since 2.2) -- This interface allows accessing all abandoned event publications and provides an API to purge them from the database, either matching a given predicate (`deletePublications(Predicate)`) or older than a given `Duration` (`deletePublicationsOlderThan(Duration)`), as described in <<publication-registry.lifecycle.abandoning>>.
...
The configured completion mode also governs how publications marked xref:events.adoc#publication-registry.lifecycle.abandoning[`ABANDONED`] are persisted, mirroring the same `UPDATE`, `DELETE`, or `ARCHIVE` strategy used for completed ones.

[[publication-registry.publication-repositories]]
=== Event Publication Repositories

To actually write the event publication log, Spring Modulith exposes an `EventPublicationRepository` SPI and implementations for popular persistence technologies that support transactions, like JPA, JDBC, Couchbase and MongoDB.
...
The Couchbase-based implementation will need collections and indexes for the event publication log.
For details, please consult the xref:appendix.adoc#schemas.couchbase[couchbase schemas] in the appendix.
...
Spring Modulith 2.1 introduced support event externalization through the https://outbox.namastack.io/[Namastack Outbox] and http://jobrunr.io/[JobRunr].
See the corresponding section (xref:events.adoc#externalization.namastack[Namastack], xref:events.adoc#externalization.jobrunr[JobRunr]) of the docs for details.
...
[[externalization.jobrunr]]
== JobRunr Outbox Support
Similar to the xref:events.adoc#externalization.namastack[Namastack support] to externalize events we provide support for http://jobrunr.io/[JobRunr].
...
For more information on how to JobRunr please consult its https://www.jobrunr.io/en/documentation/[reference documentation].
Find an https://github.com/spring-projects/spring-modulith/tree/main/spring-modulith-examples/spring-modulith-example-outbox[example] in our Github repository.
