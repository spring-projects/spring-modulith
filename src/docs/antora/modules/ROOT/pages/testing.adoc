...
If those contain bean references that cross module boundaries, the bootstrap will fail if those other modules are not included in the test run (see xref:testing.adoc#testing.bootstrap-modes[Bootstrap Modes] for details).
...
Especially if the integration of those is based on xref:events.adoc#events.aml[asynchronous, transactional event handling], dealing with the concurrent execution can be subject to subtle errors.
