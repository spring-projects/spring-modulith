...
Find out about how to monitor that in xref:20-verification.adoc[Verifying Application Module Structure].
...
By default and as described in xref:10-fundamentals.adoc#fundamentals.modules.advanced[Advanced Application Modules], an application module's base package is considered the API package and thus is the only package to allow incoming dependencies from other modules.
